...
Function SetSellStage(int _sell_stage)
	If (PAHPersEvenToned == None)
		PAHPersEvenToned = Game.GetFormFromFile(0x00889F, "paradise_halls.esm") As Faction
	EndIf
	...
	elseif sell_stage == 80
		...
		Alias_slave_up_for_sale.GetActorRef().RemoveFromFaction(PAHPersEvenToned)
	;	Alias_slave_up_for_sale.GetActorRef().RemoveFromAllFactions()
	...
	;	Alias_slave_up_for_sale.GetActorRef().DeleteWhenAble()
