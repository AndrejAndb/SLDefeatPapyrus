...
Int Property vLevel
	...
	Function set(int value)
		...
		EndIf
		If value > __vLevel && value < PlayerRef.GetLevel()
			__vLevel = value
		EndIf
	EndFunction
EndProperty
...
Event OnActivate(ObjectReference akActionRef)
	...
	if akActionRef == PlayerRef
		if !PlayerRef.IsInFaction(PAH.PAH_Player_gender)
			PAH.CheckPlayerGender(PlayerRef)
		EndIf
		if !the_actor.IsInFaction(PAH.PAH_gender)
			PAH.CheckGender(the_actor)
		EndIf
...
Function SetAccompanyingPlayer()
	if !PlayerRef
		PlayerRef = Game.GetPlayer()
	EndIf
	accompanying_player = (current_action == "follow") && (target == PlayerRef)
EndFunction
...
State stand_still
	Function StartAction()
		the_actor.AddToFaction(PAHAAStandStill)
	EndFunction

	Function EndAction()
		the_actor.RemoveFromFaction(PAHAAStandStill)
	EndFunction
EndState
...
State follow
	Function StartAction()
		the_actor.AddToFaction(PAHAAFollow)
	EndFunction

	Function EndAction()
		the_actor.RemoveFromFaction(PAHAAFollow)
	EndFunction
EndState
...
State flee_from
	Function StartAction()
		the_actor.AddToFaction(PAHAAFleeFrom)
	EndFunction

	Function EndAction()
		the_actor.RemoveFromFaction(PAHAAFleeFrom)
	EndFunction
EndState
...
State sandbox
	Function StartAction()
		the_actor.AddToFaction(PAHAASandboxAtTarget)
	EndFunction

	Function EndAction()
		the_actor.RemoveFromFaction(PAHAASandboxAtTarget)
	EndFunction
EndState
...
State Sweeping
	Function StartAction()
		the_actor.AddToFaction(PAH.PAHAASweepingAtTarget)
	EndFunction

	Function EndAction()
		the_actor.RemoveFromFaction(PAH.PAHAASweepingAtTarget)
	EndFunction
EndState
...
Function CrossArms()
	If (CanIdle() && !the_actor.IsInFaction(PAHBETied))
...
;			the_actor.SetRelationshipRank(PlayerRef, 4)
			the_actor.StopCombatAlarm()
			the_actor.Activate(PlayerRef, abDefaultProcessingOnly = true)
			in_dialogue = true
			SetCanIdle(True)
...
Function SetNakedState()
	if (PAH.DD_Suit != None) && (PAH.DD_StraitJacket != None)
		naked = the_actor.WornHasKeyword(PAHCountsAsNaked) || !(the_actor.WornHasKeyword(PAH.DD_StraitJacket) || the_actor.WornHasKeyword(PAH.DD_Suit) || the_actor.WornHasKeyword(ArmorCuirass) || the_actor.WornHasKeyword(ClothingBody))
	elseif PAH.DD_Suit != None
		naked = the_actor.WornHasKeyword(PAHCountsAsNaked) || !(the_actor.WornHasKeyword(PAH.DD_Suit) || the_actor.WornHasKeyword(ArmorCuirass) || the_actor.WornHasKeyword(ClothingBody))
	else
...
Function EquipInventory(bool equip_collar = true)
	...
	while i > 0
		...
		If (PAH.DD_Locks != None)
			if (the_form.HasKeyword(PAHRestraint) || the_form.HasKeyword(PAH.DD_Collar) || the_form.HasKeyword(PAH.DD_Locks))
				the_actor.EquipItem(the_form, true)
				if (the_form.HasKeyword(PAH.DD_Gag)) || (the_actor.WornHasKeyword(PAH.DD_Gag))
					If (Game.GetModByName("Dawnguard.esm") != 255)
						If ((PAH.DLC1ThrallFaction != None) && the_actor.IsInFaction(PAHPersNord))
							If the_actor.IsInFaction(PAH.DLC1ThrallFaction)
								the_actor.RemoveFromFaction(PAH.DLC1ThrallFaction)
							EndIf
						EndIf
					EndIf
				EndIf
...
Event OnItemAdded(Form akBaseItem, int aiItemCount, ObjectReference akItemReference, ObjectReference akSourceContainer)
	...
	if akSourceContainer == PlayerRef as ObjectReference
		...
		If ((kwDeviousInventory != None) && (akBaseItem.HasKeyword(kwDeviousInventory)))
			...
			;If ((PAH.DD_Inventory != None) && (akBaseItem.HasKeyword(PAH.DD_Inventory)))
			;	If (!(PAH.DD_Collar != None) && (akBaseItem.HasKeyword(PAH.DD_Collar)))
				bForceEquip = False
			EndIf
		...
		if (bForceEquip)
			...
			if item_as_armor && item_as_armor.getFormID()
				...
				if item_already_in_slot && item_already_in_slot.getFormID()
					If (PAH.DD_Inventory != None)
						if (!item_already_in_slot.HasKeyword(PAHRestraint) && !item_already_in_slot.HasKeyword(PAH.DD_Locks) && !item_already_in_slot.HasKeyword(PAH.DD_Inventory))
	...
	If ((PAH.DD_Gag != None) && (Game.GetModByName("Dawnguard.esm") != 255))
		if (akBaseItem.HasKeyword(PAH.DD_Gag) || the_actor.WornHasKeyword(PAH.DD_Gag))
			If ((PAH.DLC1ThrallFaction != None) && the_actor.IsInFaction(PAHPersNord))
				the_actor.RemoveFromFaction(PAH.DLC1ThrallFaction)
			EndIf
		EndIf
...
Event OnItemRemoved(Form akBaseItem, int aiItemCount, ObjectReference akItemReference, ObjectReference akDestContainer)
	...
	If ((PAH.DD_Gag != None) && (Game.GetModByName("Dawnguard.esm") != 255))
		if (akBaseItem.HasKeyword(PAH.DD_Gag) && the_actor.WornHasKeyword(PAH.DD_Gag))
			If ((PAH.DLC1ThrallFaction != None) && the_actor.IsInFaction(PAHPersNord))
				If !the_actor.IsInFaction(PAH.DLC1ThrallFaction)
					the_actor.AddToFaction(PAH.DLC1ThrallFaction)
				EndIf
			EndIf
		EndIf
	EndIf
...
Bool Function HasLeashEquipped()
	If ((PAH.zbf_NoSprint != None) && (PAH.DD_Collar != None) && (PAH.DD_AnkleShackles != None) && (PAH.SCB_Crawler != None))
		return (the_actor.WornHasKeyword(PAH.zbf_NoSprint) || the_actor.WornHasKeyword(PAH.zbf_SlowMove) || the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAH.DD_AnkleShackles) || the_actor.WornHasKeyword(PAH.DD_ForcedWalk) || the_actor.WornHasKeyword(PAH.DD_HobbleSkirt) || the_actor.WornHasKeyword(PAH.DD_PetSuit) || the_actor.WornHasKeyword(PAH.SCB_Crawler) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf ((PAH.zbf_NoSprint != None) && (PAH.DD_Collar != None) && (PAH.DD_AnkleShackles != None))
		return (the_actor.WornHasKeyword(PAH.zbf_NoSprint) || the_actor.WornHasKeyword(PAH.zbf_SlowMove) || the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAH.DD_AnkleShackles) || the_actor.WornHasKeyword(PAH.DD_ForcedWalk) || the_actor.WornHasKeyword(PAH.DD_HobbleSkirt) || the_actor.WornHasKeyword(PAH.DD_PetSuit) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf ((PAH.zbf_NoSprint != None) && (PAH.DD_Collar != None) && (PAH.SCB_Crawler != None))
		return (the_actor.WornHasKeyword(PAH.zbf_NoSprint) || the_actor.WornHasKeyword(PAH.zbf_SlowMove) || the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAH.SCB_Crawler) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf ((PAH.DD_Collar != None) && (PAH.DD_AnkleShackles != None) && (PAH.SCB_Crawler != None))
		return (the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAH.DD_AnkleShackles) || the_actor.WornHasKeyword(PAH.DD_ForcedWalk) || the_actor.WornHasKeyword(PAH.DD_HobbleSkirt) || the_actor.WornHasKeyword(PAH.DD_PetSuit) || the_actor.WornHasKeyword(PAH.SCB_Crawler) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf (PAH.zbf_NoSprint != None) && (PAH.DD_Collar != None)
		return (the_actor.WornHasKeyword(PAH.zbf_NoSprint) || the_actor.WornHasKeyword(PAH.zbf_SlowMove) || the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf (PAH.zbf_NoSprint != None) && (PAH.SCB_Crawler != None)
		return (the_actor.WornHasKeyword(PAH.zbf_NoSprint) || the_actor.WornHasKeyword(PAH.zbf_SlowMove) || the_actor.WornHasKeyword(PAH.SCB_Crawler) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf (PAH.DD_Collar != None) && (PAH.DD_AnkleShackles != None)
		return (the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAH.DD_AnkleShackles) || the_actor.WornHasKeyword(PAH.DD_ForcedWalk) || the_actor.WornHasKeyword(PAH.DD_HobbleSkirt) || the_actor.WornHasKeyword(PAH.DD_PetSuit) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf (PAH.DD_Collar != None) && (PAH.SCB_Crawler != None)
		return (the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAH.SCB_Crawler) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf (PAH.zbf_NoSprint != None)
		return (the_actor.WornHasKeyword(PAH.zbf_NoSprint) || the_actor.WornHasKeyword(PAH.zbf_SlowMove) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf (PAH.DD_Collar != None)
		return (the_actor.WornHasKeyword(PAH.DD_Collar) || the_actor.WornHasKeyword(PAH.DD_Blindfold) || the_actor.WornHasKeyword(PAHRestraintLeash))
	ElseIf (PAH.SCB_Crawler != None)
		return (the_actor.WornHasKeyword(PAH.SCB_Crawler) || the_actor.WornHasKeyword(PAHRestraintLeash))
	Else
		return the_actor.WornHasKeyword(PAHRestraintLeash)
	EndIf
EndFunction
...
Bool Function PlayerIsFacing()
	if !PlayerRef
		PlayerRef = Game.GetPlayer()
	EndIf
	float heading_angle = PlayerRef.GetHeadingAngle(the_actor)
...
Int Function getSex()
	return the_actor.GetLeveledActorBase().GetSex()
EndFunction
