;BEGIN FRAGMENT CODE - Do not edit anything between this and the end comment
;NEXT FRAGMENT INDEX 1
Scriptname dcur_leon_setstage1100 Extends TopicInfo Hidden

;BEGIN FRAGMENT Fragment_0
Function Fragment_0(ObjectReference akSpeakerRef)
Actor akSpeaker = akSpeakerRef as Actor
;BEGIN CODE
	GetOwningQuest().SetObjectiveCompleted(240)
	GetOwningQuest().SetObjectiveDisplayed(240)
	GetOwningQuest().SetStage(1100)	
	akSpeaker.SetRestrained(False)
	akSpeaker.SetDontMove(false)
	dclibs.WipeRestraints(Game.GetPlayer(), genericonly = false, removeDCURquestitems = true, destroyDevices = true)					
	leqs.leon_questrunning = false
;END CODE
EndFunction
;END FRAGMENT

;END FRAGMENT CODE - Do not edit anything between this and the begin comment

dcur_library Property dclibs  Auto  
dcur_leon_enslaveScript Property leqs Auto 
