Scriptname dcur_lb_vibratorScript extends zadEquipScript
...
Function OnEquippedPre(actor akActor, bool silent=false)
	...
	if akActor == libs.PlayerRef
		if Aroused.GetActorExposure(akActor) < libs.ArousalThreshold("Desire")
			msg = "Your pussy is now filled, as is your desire for pleasure."
		elseif Aroused.GetActorExposure(akActor) < libs.ArousalThreshold("Horny")
			msg = "You slowly insert the vibrator inside your pussy, your lust growing with every inch they slide in."
		elseif Aroused.GetActorExposure(akActor) < libs.ArousalThreshold("Desperate")
			msg = "You insert the vibrator inside your wet pussy and take great delight in the resulting feelings of pleasure."
		else
			msg = "Barely in control of control your own body you thrust the vibrator almost forcefully into your longing, wet pussy."
		endif
	else
		msg = akActor.GetLeveledActorBase().GetName() + " shudders as you push the vibrator into her."
	EndIf
	if !silent
...
Function DeviceMenu(Int msgChoice = 0)
	...
	elseif msgChoice==2 ; Not wearing a belt, plugs		
		if !lbqs.lb_hasremoved_Catsuit
			libs.Notify("You cannot remove the vibrator while wearing a rubber suit!", messageBox=true)		
			return
		endif
		string msg = ""
...
Function NoKeyFailMessage(Actor akActor)
	if akActor == libs.PlayerRef
		libs.NotifyPlayer("Try as you might, the belt you are wearing prevents you from removing the vibrator.", true)
	Else
